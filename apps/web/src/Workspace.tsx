...
export default function Workspace() {
  ...
  return <div className="app-layout"><a className="skip-link" href="#workspace-main">Ir para o conteúdo</a><aside className="sidebar"><a className="brand" href="#inbox"><span className="brand-mark" aria-hidden="true">jh</span><span>JobHunter <span className="brand-ai">AI</span></span></a><p className="eyebrow">ESPAÇO DE TRABALHO</p><nav aria-label="Navegação principal"><a href="#inbox" aria-current={route === 'inbox' ? 'page' : undefined}>Inbox de oportunidades</a><a href="#profile" aria-current={route === 'profile' ? 'page' : undefined}>Perfil e evidências</a><a href="#import" aria-current={route === 'import' ? 'page' : undefined}>Importar vaga</a></nav><p className="sidebar-note">Você decide cada passo.<br />Dados no ambiente local.</p></aside><main id="workspace-main" tabIndex={-1} className="app-main">{error ? <ErrorState error={error} retry={() => { void refresh().catch((reason: Error) => setError(reason.message)); }} /> : !data ? <Loading /> : route === 'profile' ? <ProfilePage {...data} refresh={refresh} /> : route === 'import' ? <ImportJob /> : route.startsWith('job/') ? <JobDetail key={route} id={route.slice(4)} profile={data.profile} facts={data.facts} /> : <Inbox />}</main></div>;
}
